...
void Game::make_a_character_shoot(Character *character)
{
    ...
    {
        Gunshot *newGunshot = new Gunshot(character->get_id(), character->get_center().x - (character->get_trunk_width() / 2 * 1.1)*sin(character->get_theta_body()/180*M_PI), character->get_center().y, character->get_center().z + (character->get_trunk_width() / 2 * 1.1)*cos(character->get_theta_body()/180*M_PI), character->get_speed(), character->get_radius(), character->get_theta_arm() + (90), character->get_theta_body(), character->get_facing_direction(), character->get_crewmate_colors().upperBody);
